...
function getRequestParamsData(
    config: AxiosRequestConfig,
    responseStatus = 0
): Array<IRequestParamsData | string | undefined> {
    if (config) {
        const requestParamsData: IRequestParamsData = {}
        const status: string =
            responseStatus > 0 && Math.floor(responseStatus / 100) !== 2
                ? `(${responseStatus}) `
                : ''
        const configUrl = config.url ?? ''
        const url: string =
            configUrl.startsWith('http') || configUrl.startsWith('//')
                ? configUrl
